...
export default function JournalV2() {
  ...
  const panel = (t: TradeWithTags) =>
    openId !== t.id ? null : (
      ...
      >
        {/* No header of its own: TradeBody's is the sticky one and carries
            the way out. A bar here as well put the ticker on screen three
            times — row, bar, header — with two sets of the same buttons. */}
        {editingId === t.id ? (
          /* Cancel returns to the trade rather than closing the panel, which
             is the right destination: you came from the trade. */
